...
public class ProgramGenerator extends CodeGenerator
{
     ...
     */
    public void emitProgram(PascatParser.PawgramContext ctx)
    {
    	programId = ctx.pawgramHeader().pawgramIdentifier().entry;
     ...
     */
    private void emitSubroutines(PascatParser.RoutinesPartContext ctx)
    {
        if (ctx != null)
        {
            for (PascatParser.RoutineDefinitionContext defnCtx : 
     ...
     * @param ctx the ProgramContext.
     */
    private void emitMainMethod(PascatParser.PawgramContext ctx)
    {
        emitLine();
        emitComment("MAIN");
     ...
     */
    public void emitRoutine(PascatParser.RoutineDefinitionContext ctx)
    {
        SymtabEntry routineId = ctx.pawcedureHead() != null 
                                ? ctx.pawcedureHead().routineIdentifier().entry
