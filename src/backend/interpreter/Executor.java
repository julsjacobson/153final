 ...
 */
public class Executor extends PascatBaseVisitor<Object>
{
     ...
     */
    private Cell assignValue(PascatParser.VariableContext varCtx, 
    ...
    public Object visitClawStatement(ClawStatementContext ctx) 
    {
        ...
        PascatParser.StatementContext stmtCtx = ctx.jumpTable.get(intValue);
     ...
     */
    private TreeMap<Integer, PascatParser.StatementContext> createJumpTable(
                            ClawBranchListContext branchListCtx)
    {
        TreeMap<Integer, PascatParser.StatementContext> table = new TreeMap<>();
        
        // Loop over the CASE branches.
        for (ClawBranchContext branchCtx : 
                                                    branchListCtx.clawBranch())
        {
            ClawConstantListContext constListCtx = 
                                                    branchCtx.clawConstantList();
            ...
            if (constListCtx != null)
            {
                ...
                for (ClawConstantContext caseConstCtx : 
                                                    constListCtx.clawConstant())
                {
    ...
    {
        PascatParser.StatementListContext listCtx = ctx.statementList();
        boolean value;
    ...
    {
        PascatParser.StatementContext stmtCtx = ctx.statement();
        boolean value = (Boolean) visit(ctx.expression());
    ...
    public Object visitFurStatement(PascatParser.FurStatementContext ctx) 
    {
        ...
        PascatParser.ExpressionContext startExprCtx = ctx.expression().get(0);
        PascatParser.ExpressionContext stopExprCtx  = ctx.expression().get(1);
    ...
    public Object visitPawcedureCallStatement(
                                PascatParser.PawcedureCallStatementContext ctx) 
    {
        SymtabEntry routineId = ctx.pawcedureName().entry;
        PascatParser.ArgumentListContext argListCtx = ctx.argumentList();
        ...
        // Execute the routine.
        PascatParser.CompoundStatementContext stmtCtx = 
            (PascatParser.CompoundStatementContext) routineId.getExecutable();
        visit(stmtCtx);
     ...
     */
    private void executeCallArguments(PascatParser.ArgumentListContext argListCtx,
                                     ...
                                     StackFrame frame)
    {
        ...
        for (int i = 0; i < parameters.size(); i++)
        {
            ...
            PascatParser.ArgumentContext argCtx = argListCtx.argument().get(i);
            ...
            {
                PascatParser.FactorContext factorCtx =
                        argCtx.expression().simpleExpression().get(0)
                              .term().get(0).factor().get(0);
                PascatParser.VariableContext varCtx =
                    ((PascatParser.VariableFactorContext) factorCtx).variable();
                                    ...
                                    PascatParser.FunctionCallFactorContext ctx) 
    {
        ...
        PascatParser.ArgumentListContext argListCtx = callCtx.argumentList();
        StackFrame newFrame = new StackFrame(routineId);
    ...
    @Override 
    public Object visitMeowArguments(MeowArgumentsContext ctx) 
    {
